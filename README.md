...
**Redis cache for generated HTTP pages**  enable `httpCache` on HTTP services whose responses are generated by plugins. Cached responses are keyed by method, host, request URI, and request body hash, so repeated sqlmap probes against the same address receive the same status, headers, and body.
